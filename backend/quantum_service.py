...
from .config import settings, get_backend_config
...
try:  # pragma: no cover - used only when Qiskit is present
    if settings.USE_QISKIT_MOCK:
        raise ImportError("Using mock classes")
    ...
    class MockQuantumCircuit:  # type: ignore
    ...
    class MockAerSimulator:  # type: ignore
        pass

    class MockNoiseModel:  # type: ignore
        pass

    class MockOperator:  # type: ignore
        pass

    class MockResult(dict):  # type: ignore
        def get_counts(self) -> Dict[str, int]:
            return {}

    def execute(*args: Any, **kwargs: Any) -> MockResult:  # type: ignore
        return MockResult()

    def transpile(
        circuit: MockQuantumCircuit, *args: Any, **kwargs: Any
    ) -> MockQuantumCircuit:  # type: ignore
        return circuit
    ...
    class MockCompleteMeasFitter:  # type: ignore
        def __init__(self, *args: Any, **kwargs: Any) -> None:
            pass

        class filter:  # noqa: D401 - simple wrapper
            @staticmethod
            def apply(result: MockResult) -> MockResult:
                return result
    ...
    QuantumCircuit = MockQuantumCircuit  # type: ignore
    AerSimulator = MockAerSimulator  # type: ignore
    NoiseModel = MockNoiseModel  # type: ignore
    Operator = MockOperator  # type: ignore
    Result = MockResult  # type: ignore
    CompleteMeasFitter = MockCompleteMeasFitter  # type: ignore
...
class QuantumService:
            ...
            if settings.ERROR_MITIGATION_METHOD == "zne":
                ...
                for noise_level in noise_levels:
                    ...
                    result_noisy = execute(
                        circuit, self.simulator, noise_model=noise_model, shots=1000
                    ).result()
                ...
                meas_calibs, state_labels = complete_meas_cal(
                    qubit_list=range(circuit.num_qubits), circlabel="mcal"
                )

                # Execute calibration circuits
                cal_results = execute(meas_calibs, self.backend, shots=1000).result()
    ...
    def _extrapolate_zero_noise(
        self, results: List[Result], noise_levels: List[float]
    ) -> Result:
