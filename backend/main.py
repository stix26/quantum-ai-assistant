...
# The heavy Qiskit runtime is optional and may not be available in
# lightweight environments. The `quantum_service` helper will handle
# initialization when possible, so we simply create a placeholder here.
service = None
...
def generate_suggested_topics(metrics: Dict[str, float]) -> List[str]:
    """Return topic suggestions based on available quantum metrics."""
    topics: List[str] = []
    if metrics.get("entanglement", 0.0) > 0.7:
...
def generate_next_actions(
    ...
    if "?" in message:
        actions.append(
            {
                "type": "explanation",
                "confidence": metrics.get("purity", 0.0),
    ...
    if "quantum" in message.lower():
        actions.append(
            {
                "type": "visualization",
                "confidence": metrics.get("coherence", 0.0),
